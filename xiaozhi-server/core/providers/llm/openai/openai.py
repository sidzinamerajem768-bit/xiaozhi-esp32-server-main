...
class LLMProvider(LLMProviderBase):
    ...
    def normalize_dialogue(dialogue):
        ...
        for msg in dialogue:
            ...
            if isinstance(content, str):
                msg["content"] = content.encode("utf-8", errors="replace").decode("utf-8")
        return dialogue
    ...
    def response(self, session_id, dialogue, **kwargs):
        logger.bind(tag=TAG).info(f"[LLM-OPENAI] model={self.model_name} base_url={self.base_url}")
    ...
    def response_with_functions(self, session_id, dialogue, functions=None, **kwargs):
        logger.bind(tag=TAG).info(f"[LLM-OPENAI-FC] model={self.model_name} base_url={self.base_url}")
        dialogue = self.normalize_dialogue(dialogue)
