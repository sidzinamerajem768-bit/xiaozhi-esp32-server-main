...
class MemoryProvider(MemoryProviderBase):
    ...
    @staticmethod
    def _safe_powermem_call(func, **kwargs):
        safe_kwargs = {}
        for k, v in kwargs.items():
            if isinstance(v, str):
                safe_kwargs[k] = v.encode("utf-8", errors="replace").decode("utf-8")
            elif isinstance(v, list) and all(isinstance(m, dict) for m in v):
                safe_kwargs[k] = [
                    {mk: (mv.encode("utf-8", errors="replace").decode("utf-8") if isinstance(mv, str) else mv)
    ...
    async def save_memory(self, msgs, session_id=None):
        ...
        try:
            if self.use_powermem and self.memory_client is not None and len(msgs) >= 2:
                ...
                result = self._safe_powermem_call(
    ...
    async def query_memory(self, query: str) -> str:
        ...
        try:
            ...
            try:
                ...
                safe_query = search_query.encode("ascii", errors="replace").decode("ascii")
                if self.enable_user_profile:
                    results = await asyncio.to_thread(
                        self.memory_client.search,
                        query=safe_query,
